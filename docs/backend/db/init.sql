...
CREATE TABLE IF NOT EXISTS carga_horas (
    ...
    recurso_id UUID NOT NULL,       -- ID del recurso  de la API externa
    tarea_id UUID NOT NULL,         -- ID de la tarea de la API externa
    proyecto_id UUID NOT NULL,      -- ID del proyecto de la API externa
    cliente_id INTEGER,    -- ID del cliente de la API externa
    fecha DATE NOT NULL,
    horas DECIMAL(5,2) NOT NULL CHECK (horas > 0 AND horas <= 24), -- No permitir horas negativas
    descripcion TEXT, 
